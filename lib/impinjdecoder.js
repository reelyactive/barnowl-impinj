  ...
  data.forEach(event => {
    ...
    if(isTagInventory) {
      ...
      if(hasTid || hasEpc) {
        ...
        let lastSeenTime = tagInventoryEvent.lastSeenTime || Date.now();
        let hasHostname = event.hasOwnProperty('hostname');
        ...
        let raddec = new Raddec({
            ...
            timestamp: new Date(lastSeenTime).getTime()
        });
        ...
        if(hasHostname) {
          ...
          raddec.addDecoding({
              ...
              receiverAntenna: tagInventoryEvent.antennaPort,
